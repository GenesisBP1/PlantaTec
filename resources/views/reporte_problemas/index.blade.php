...
<head>
    ...
    <title>PlantaTec — Gestión de reportes de problemas</title>
    ...
    <style>
        /* ========== ESTILOS GLOBALES PLANTA TEC (proporcionados por el usuario) ========== */
        ...
        * {
        ...
        /* Estilos específicos para la tabla de administración de reportes */
        .pt-admin-card {
            background: #ffffff;
            border: 1px solid #e5e7eb;
            border-radius: 1.5rem;
                    ...
                    <table class="pt-admin-table">
                        ...
                        <tbody>
                            @forelse($reportes as $reporte)
                                <tr>
                                    <td>{{ $reporte->adopcion->usuario->name ?? 'N/A' }}</td>
                                    <td>{{ $reporte->adopcion->planta->nombre ?? 'N/A' }}</td>
                                    <td>{{ $reporte->problema->nombre ?? 'N/A' }}</td>
                                    <td>{{ ucfirst($reporte->gravedad) }}</td>
                                    <td>{{ ucfirst(str_replace('_', ' ', $reporte->estado)) }}</td>
                                    <td>{{ $reporte->created_at->format('d/m/Y') }}</td>
                                    <td class="pt-table-actions">
                                        <a href="{{ route('reporte-problemas.show', $reporte) }}" class="pt-action-btn view">Ver</a>
                                        @if(auth()->user()->rol === 'admin')
                                            <a href="{{ route('reporte-problemas.edit', $reporte) }}" class="pt-action-btn edit">Editar</a>
                                            <form action="{{ route('reporte-problemas.destroy', $reporte) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
