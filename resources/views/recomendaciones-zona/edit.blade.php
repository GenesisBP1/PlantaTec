<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ✏️ Editar zona: {{ $zona->nombre_lugar }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl p-6">
                <form action="{{ route('recomendaciones-zona.update', $zona) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nombre_lugar" value="{{ old('nombre_lugar', $zona->nombre_lugar) }}" 
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="indicaciones" rows="3" class="w-full border-gray-300 rounded-lg shadow-sm">{{ old('indicaciones', $zona->indicaciones) }}</textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="descripcion" rows="4" class="w-full border-gray-300 rounded-lg shadow-sm">{{ old('descripcion', $zona->descripcion) }}</textarea>
                    </div>

                    <div class="flex justify-end gap-2">
                        <a href="{{ route('recomendaciones-zona.index') }}" class="bg-gray-400 text-white px-4 py-2 rounded-lg">Cancelar</a>
                        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg">Actualizar</button>
                    </div>
                </form>
            </div>
        </div>
