<x-app-layout>
    ...
    <div class="py-8">
        <div class="adopciones-container">
            ...
            <!-- Filtros -->
            <form method="GET" action="{{ route('adopciones.index') }}" class="filtros">
                <select name="estado">
                ...
                <a href="{{ route('adopciones.index') }}">Limpiar</a>
            </form>
            ...
            <div class="plantas-list">
                @forelse($adopciones as $adopcion)
                    ...
                    <div class="planta-card">
                        <a href="{{ route('adopciones.show', $adopcion) }}">
                            <img class="planta-img" src="{{ $planta->imagen ?? 'https://images.unsplash.com/photo-1592150621744-aca64f48394a?w=200&h=150&fit=crop' }}" alt="{{ $planta->nombre }}">
                        </a>
                        
                        <div class="info-planta">
                            <h3 class="nombre-planta">
                                <a href="{{ route('adopciones.show', $adopcion) }}">{{ $planta->nombre }}</a>
                            </h3>
                            ...
                            <div class="cuidados-list">
                                @foreach($cuidadosAsignados as $pc)
                                    <span class="cuidado-badge">
                                        {{ $pc->cuidado->nombre }} (cada {{ $pc->frecuencia }} días)
                                    </span>
                                @endforeach
                            </div>
                            ...
                            @if($ultimoRegistro)
                                <div class="ultima-evidencia">
                                    @if($ultimoRegistro->imagen)
                                        <img src="{{ Storage::url($ultimoRegistro->imagen) }}" alt="evidencia">
                                    @else
                        ...
                        <div class="acciones">
                            <button class="btn-evidencia" data-adopcion-id="{{ $adopcion->id }}" data-planta-nombre="{{ $planta->nombre }}">
                                <i class="fas fa-camera"></i> Subir evidencia
                            </button>

                            <a href="{{ route('reporte-problemas.create', ['adopcion_id' => $adopcion->id]) }}" class="btn-problema">
                ...
                @empty
                    <div class="bg-white rounded-2xl p-10 text-center text-gray-500">
                        No tienes plantas adoptadas aún. <a href="{{ route('catalogo.plantas') }}" class="text-green-600">¡Adopta una!</a>
                    </div>
                @endforelse
            </div>
        </div>
