...
<div id="info-marcador-{{ $id }}" class="hidden mt-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
    <div class="space-y-2">
    ...
    <div class="mt-3 flex gap-2">
        <button type="button" onclick="cerrarInfoMarcador('{{ $id }}')" class="flex-1 px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg transition">
            Cerrar
        </button>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar mapa
    const mapa = L.map('{{ $id }}').setView([25.5095, -97.1559], 13); // Matamoros, Tamaulipas
...
// Cargar ubicaciones existentes
    ...
    const botonGeo = document.querySelector(`[data-mapa-id="{{ $id }}"]`);
    if (botonGeo) {
    ...
    const botonBuscar = document.querySelector(`[class*="buscar-ubicacion-btn"][data-mapa-id="{{ $id }}"]`);
    if (botonBuscar) {
